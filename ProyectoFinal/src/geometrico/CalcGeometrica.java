...
public class CalcGeometrica {
    ...
    /*public static void main(String[] args) {
		  ...
		  System.out.println(cuadArea(6,3,4,-1));
	...
	public double rectArea(double x1,double x2, double x3, double y1, double y2, double y3){//Vx3
		...
		double x4,y4,aux1,aux2;
		
		aux1= x2-x1;  aux2 = y2-y1; //Vector que union entre los Vertices
		
		x4 = x2 + aux1; y4 = y2 + aux2; // V4 Encontrado
		
		//Para calcular formula de Hamen
		s1 = distanciaAB(x1,x2,y1,y2); // x2 va  a estar en -y como punto de referencia el orden no importa
		s2 = distanciaAB(x2,x3,y2,y3); // x3 es el que le sigue a B
		s3 = distanciaAB(x1,x4,y1,y4);
	...
	public static double cuadArea(double x1,double x2, double y1, double y2){//Vx2
	...
	public static double distanciaAB(double x1, double x2, double y1, double y2 ){
	
		double sn;
		
		sn = Math.sqrt( Math.pow((x2 - x1),2) + Math.pow((y2 - y1),2) );
